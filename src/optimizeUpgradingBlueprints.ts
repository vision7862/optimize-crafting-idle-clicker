...
export function upgradeMostImpactfulResearchSet(blueprints: Blueprint[] = BLUEPRINT_LIBRARY): SetUpgradeInfo | null {
  let bestROI = 0;
  let bestUpgradeInfo: SetUpgradeInfo | null = null;
  let setName = '';
  BLUEPRINT_SETS.filter((set: BlueprintSet) => set.multiplierType === SetMultiplierType.Research).forEach(
    (set: BlueprintSet) => {
      const distanceInfo = getDistanceToNextRank(set, convertBlueprintLibraryToScores(blueprints));
      const upgradeInfo: SetUpgradeInfo | null = upgradeSetToNextRank(set, blueprints);
      if (upgradeInfo !== null) {
...
export function printUpgradeInfoOfEachSet(blueprints: Blueprint[] = BLUEPRINT_LIBRARY): void {
  BLUEPRINT_SETS.forEach((set: BlueprintSet) => {
    ...
    if (upgradeInfo !== null) {
      ...
      //   }`,
      // );
      console.log(`${set.setName};${roi};${upgradeInfo.cost};${SetMultiplierType[set.multiplierType]}`);
    }
  });
}
